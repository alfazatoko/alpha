...
let file = fs.readFileSync('c:/Users/Administrator/Desktop/ALFAZA CELL/APLIKASI BARU/ALPHA(apk-toko)/src/components/TransactionForm.tsx', 'utf8');

// 1. Replace Auto Keterangan Logic
file = file.replace(
  /\/\/ Auto Keterangan Logic[\s\S]*?setKeterangan\(autoText\.toUpperCase\(\)\)\r?\n\s*\}, \[isKetAuto[^\]]+\]\);/,
...
// 2. Replace onSaveInternal
file = file.replace(
  /const onSaveInternal = \(\) => \{[\s\S]*?setIsKetAuto\(true\)\r?\n\s*\}/,
...
// 3. Replace Keterangan UI & Presets
const oldKeteranganUI = \`<div className="relative group px-2">
...
const newKeteranganUI = \`<div className="relative group px-2">
          <div className="flex justify-between items-center mb-1 px-1">
            <label className="block text-[10px] font-black text-gray-700 uppercase tracking-widest flex items-center gap-1.5">
              <i className="fa-solid fa-align-left text-gray-400"></i> Keterangan
            </label>
...
file = file.replace(oldKeteranganUI, newKeteranganUI);

fs.writeFileSync('c:/Users/Administrator/Desktop/ALFAZA CELL/APLIKASI BARU/ALPHA(apk-toko)/src/components/TransactionForm.tsx', file);
