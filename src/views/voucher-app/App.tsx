...
export default function App({ onExit, externalRole, externalCashierName, activeStoreId, googleUid, kasirList, externalSearchQuery, externalTab, onClearExternalTab, onClearExternalSearchQuery, storeList }: VoucherAppProps = {}) {
  ...
  useEffect(() => {
    ...
    const fetchSQLData = async () => {
      try {
        ...
        // Fetch Stocks
        const { data: stocks, error: err2 } = await supabase.from('voucher_stocks').select('*').eq('store_id', activeStoreId).eq('cashier_id', cashierId);
  ...
  const handleExecuteShiftHandover = (
    ...
    if (activeStoreId && !isSelfHandover) {
      const fromCashierId = activeCashier.id;
      
      const handoverUpserts = finalProducts.map(p => ({
        store_id: activeStoreId,
        product_id: p.id,
        cashier_id: toCashierId,
        current_stock: p.currentStock // Overwrite entirely as requested by user
      }));
      
      const zeroUpserts = finalProducts.map(p => ({
        store_id: activeStoreId,
        product_id: p.id,
        cashier_id: fromCashierId,
        current_stock: 0 // Empty the sender's stock
      }));
      
      supabase.from('voucher_stocks').upsert([...handoverUpserts, ...zeroUpserts], { onConflict: 'store_id,product_id,cashier_id' }).then(res => console.log('Upsert handover:', res));
    }
